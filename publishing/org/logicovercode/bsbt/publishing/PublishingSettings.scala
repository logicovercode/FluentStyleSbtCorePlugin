...
trait PublishingSettings extends PublishingModel {
  ...
  implicit class ModuleBuildArtifactPublishExtension(moduleBuild: ModuleBuild) {
    ...
    def argsRequiredForPublishing(projectDevelopers: List[Developer], license: License, homePageUrl : URL,
                                  moduleScmInfo : ScmInfo, mavenRepository: MavenRepository): ModuleBuild = {
